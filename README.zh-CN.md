...
TreeWork 是一个面向 Codex 的**状态原生项目记忆（state-native project
memory）插件**：它显式、可查询地维护已经接受的项目组织、设计、各 branch 的
实际状态、验证证据和开发轨迹。

代码检查能够看到已经实现了什么，检索型记忆能够找回历史片段，但两者都不能
稳定回答项目已经接受了什么、工作进行到哪里，以及上一个 Agent 为什么停下。
TreeWork 将这些运行状态保存在模型上下文之外，让长期任务不必每次从头重建项目
就能继续。
...
`v0.1.3` 是当前版本。Alignment、声明式 Tree 构建、受保护的 branch 移动、
